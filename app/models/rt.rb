class Rt < ApplicationRecord
  ...
  def self.import_insales(path_file, extend_file)
    ...
    (2..last_spreadsheet).each do |i|
      ...
      product.present? ? product.update(data) : Rt.create(data)
      p data
      p product
    end
  ...
  def self.unlinking_to_xls
    file = "#{Rails.root}/public/rts_unlinking.xls"
    ...
    sheet = book.create_worksheet(name: 'RT незалинкованные')
    sheet.row(0).push("ID", "ID в таблице Товаров", "Артикул", "Название", "Остаток", "Цена", "Описание", "Параметры")

    # products = Rt.where(rt_id: nil).order(:id)
    products = Product.all.map(&:rt_id).reject(&:nil?)
    rts_unlinks = Rt.where('id not in (?)', products)
